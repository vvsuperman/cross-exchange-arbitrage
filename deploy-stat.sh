...
# 上传并部署
scp ../cross-stat.zip $HOST:$PROJECT_PATH/

ssh "$HOST" "bash -s" << 'EOF'
    # 停止旧进程
    pkill -9 -f 'statistical_arbitrage.py'
    
    cd /home/admin/myproject
    source lumao_env/bin/activate
    
    # 准备目录
    if [ ! -d "cross-stat" ]; then
        mkdir -p cross-stat
    fi
    mv cross-stat.zip cross-stat/
    
    cd cross-stat
    rm -rf cross-exchange-arbitrage
    
    # 解压覆盖到 cross-exchange-arbitrage 目录
    unzip -o cross-stat.zip -d cross-exchange-arbitrage
    cd cross-exchange-arbitrage
    
    # 启动统计套利应用
    # 配置从 tickers.txt 文件读取标的列表
    nohup python monitor/statistical_arbitrage.py > nohup.out 2>&1 &
