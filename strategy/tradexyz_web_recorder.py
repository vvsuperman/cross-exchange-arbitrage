#!/usr/bin/env python3
"""Record trade.xyz frontend quotes into bbo_data."""
...
def _handle_shutdown(signum, _frame) -> None:
    ...
    print(f"[tradexyz_web_recorder] received signal {signum}, shutting down...")
...
def _normalize_target_config(data: dict) -> ExchangeTargetConfig:
    ...
    if not symbol:
        raise ValueError("trade_xyz symbol is required")
    if not logical_symbol:
        raise ValueError("trade_xyz logical_symbol is required")
    user_data_dir = os.path.expanduser(
        str(payload.get("user_data_dir") or XYZ_TRADER_USER_DATA_DIR).strip()
    )
    if not user_data_dir:
        raise ValueError("trade_xyz user_data_dir is required")
...
def parse_args() -> RecorderConfig:
    parser = argparse.ArgumentParser(description="Start trade.xyz browsers, then persist quotes into bbo_data")
    ...
    parser.add_argument("--auto-connect", action="store_true", help="Run trade.xyz wallet auto-connect if needed")
    parser.add_argument(
...
def _safe_quit_trade_xyz_client(client, profile: str) -> None:
    try:
        ...
        pass
    cleanup_trade_xyz_profile(profile, reason="tradexyz_web_recorder_quit")
...
def _start_runtime(target: ExchangeTargetConfig) -> TargetRuntime:
    ...
    print(
        f"[tradexyz_web_recorder] opening trade.xyz market: "
...
def _safe_quit_runtime(runtime: TargetRuntime) -> None:
    _safe_quit_trade_xyz_client(runtime.client, runtime.config.user_data_dir)
...
def main() -> int:
    ...
    for target in config.targets:
        cleanup_trade_xyz_profile(target.user_data_dir, reason="tradexyz_web_recorder_start")
        ...
        while not _SHUTDOWN_REQUESTED:
            for index, runtime in enumerate(list(runtimes)):
                try:
                    ...
                    print(
                        "[tradexyz_web_recorder] webdriver session dropped "
                    ...
                    if runtime.recovery_attempts > _MAX_SESSION_RECOVERY_ATTEMPTS:
                        print(
                            "[tradexyz_web_recorder] exceeded webdriver recovery attempts "
                            f"for logical={runtime.config.logical_symbol}; exiting"
                        )
                        return 1

                    _safe_quit_runtime(runtime)
                    cleanup_trade_xyz_profile(
                        runtime.config.user_data_dir,
                        reason="tradexyz_web_recorder_recover",
                    )
                    time.sleep(2.0)
                ...
                _print_snapshots(f"trade_xyz:{runtime.config.logical_symbol}", snapshots)
